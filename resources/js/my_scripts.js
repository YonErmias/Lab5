...
*/
	function loadStatsPage()
	{
		...
		var score;
		var win_counts = 0;
		for(i_row = 2; i_row < table.rows.length; i_row++)
		{
			homeScore = 0;
			oppScore = 0;
			score = table.rows[i_row].cells[2].innerHTML - table.rows[i_row].cells[3].innerHTML;
			if (score > 0)
			{
				table.rows[i_row].cells[4].innerHTML = 'Home';
				win_counts = win_counts + 1;
			}
			else if (score < 0)
			{
				table.rows[i_row].cells[4].innerHTML = 'Opp';
			}
			else
			{
				table.rows[i_row].cells[4].innerHTML = 'Tie';
			}
		}
		var wins = document.getElementById("wins");
		wins.innerHTML = win_counts;
		
		
		var losses = document.getElementById("losses");
		losses.innerHTML = (table.rows.length - 2 ) - win_counts;
	}
